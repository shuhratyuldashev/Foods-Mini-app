import React from "react";

interface CartCardFooterProps {
  count: number;
  setCount: React.Dispatch<React.SetStateAction<number>>;
  data: any;
  decrease: (
    e: React.MouseEvent,
    count: number,
    data: any,
    setCount: React.Dispatch<React.SetStateAction<number>>,
    onUpdate?: () => void,
  ) => void;
  increase: (
    e: React.MouseEvent,
    count: number,
    data: any,
    setCount: React.Dispatch<React.SetStateAction<number>>,
    onUpdate?: () => void,
  ) => void;
  removeFromCart: (data: any, onUpdate?: () => void) => void;
  onUpdate?: () => void;
}

const CartCardFooter = ({
  count,
  setCount,
  data,
  decrease,
  increase,
  removeFromCart,
  onUpdate,
}: CartCardFooterProps) => {
  return (
    <div className="flex justify-between w-full">
      <div className="flex items-center gap-2">
        <button
          onClick={(e) => decrease(e, count, data, setCount, onUpdate)}
          className="p-1 rounded-full border bg-white"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth={1.5}
            stroke="currentColor"
            className="size-4"
          >
            <path strokeLinecap="round" strokeLinejoin="round" d="M5 12h14" />
          </svg>
        </button>
        <p className="text-sm">{count}</p>
        <button
          onClick={(e) => increase(e, count, data, setCount, onUpdate)}
          className="p-1 rounded-full border bg-white"
        >
          <svg
            xmlns="http://www.w3.org/2000/svg"
            fill="none"
            viewBox="0 0 24 24"
            strokeWidth={1.5}
            stroke="currentColor"
            className="size-4"
          >
            <path
              strokeLinecap="round"
              strokeLinejoin="round"
              d="M12 4.5v15m7.5-7.5h-15"
            />
          </svg>
        </button>
      </div>
      <button
        className="text-red-600"
        aria-label="O‘chirish"
        onClick={(e) => {
          e.preventDefault();
          removeFromCart(data, onUpdate);
        }}
      >
        <svg
          xmlns="http://www.w3.org/2000/svg"
          fill="none"
          viewBox="0 0 24 24"
          strokeWidth={1.5}
          stroke="currentColor"
          className="size-5"
        >
          <path
            strokeLinecap="round"
            strokeLinejoin="round"
            d="m14.74 9-.346 9m-4.788 0L9.26 9m9.968-3.21
             c.342.052.682.107 1.022.166m-1.022-.165L18.16 
             19.673a2.25 2.25 0 0 1-2.244 2.077H8.084a2.25 
             2.25 0 0 1-2.244-2.077L4.772 5.79m14.456 0a48.108 
             48.108 0 0 0-3.478-.397m-12 .562c.34-.059.68-.114 
             1.022-.165m0 0a48.11 48.11 0 0 1 3.478-.397m7.5 
             0v-.916c0-1.18-.91-2.164-2.09-2.201a51.964 
             51.964 0 0 0-3.32 0c-1.18.037-2.09 1.022-2.09 
             2.201v.916m7.5 0a48.667 48.667 0 0 0-7.5 0"
          />
        </svg>
      </button>
    </div>
  );
};

export default CartCardFooter;
